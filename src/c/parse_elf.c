...
int parse_elf_my_func(const char *libpath, const char *libname, const char *objname,
                   const void *addr, const size_t size,
                   const symbol_bind binding, const symbol_type type,
...
int parse_elf_main(int argc, char *argv[])
{
    int  arg;
    ...
    if (symbols(parse_elf_my_func, NULL))
        return EXIT_FAILURE;
